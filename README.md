...
## 🔖 11월 26일 (14주차)
<h2> Recomendations(권장 사항) </h2>

- CSS import는 하나의 JavaScript 또는 TypeScript entry 파일에 포함
- 글로벌 스타일과 Tailwind 스타일시트는 애플리케이션 루트에서 import함
- Tailwind CSS는 유틸리티 클래스를 지원하기 때문에 대부분의 스타일링에 사용 추천
- 충분하지 않은 때는 컴포넌트별 스타일에 CSS module을 사용
...
- 시각적 안정성
- 더 빠른 페이지 로드
- 자산 유연성: 원격 서버에 저장된 이미지도 원하는 대로 이미지 크기를 조정
...
<h2> Vercel의 Next.js(Vercel 배포) </h2>


## 🔖 11월 19일 (13주차)
<h2> CSS Modules </h2>

- 고유한 클래스 이름을 생성하여 CSS의 범위를 로컬로 지정
- 이름 충돌에 대한 걱정 없이 다른 파일에서 동일한 클래스를 사용 가능
- .module.css 확장자가 있는 새 파일을 만들고, app 디렉토리의 컴포넌트로 가져옴

<h2> Bootstrap 실습 </h2>

- Tailwind 등 다른 프레임워크와 함께 사용하지 않는 것이 바람직함
- layout이 영향을 미치는 범위가 이상하다는 것을 발견할 수 있습니다.
- Booststrap이 전역기반 CSS 프레임워크이기 때문
- localLayout뿐만 아니라 src/의 어느 곳에서나 한 번 import하면 Next.js 번들에 합쳐져서 전역적으로 영향을 미치게 됨
